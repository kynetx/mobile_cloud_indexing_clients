...
- (BOOL)start:(NSError **)errPtr
{
	...
	dispatch_sync(serverQueue, ^{ @autoreleasepool {
		...
		if (success)
		{
            NSString *localAddress = [[NSString alloc] initWithData:[asyncSocket localAddress] encoding:NSUTF8StringEncoding];
			HTTPLogInfo(@"%@: Started HTTP server on port %hu with local host:%@  local address:%@", THIS_FILE, [asyncSocket localPort], [asyncSocket localHost], localAddress);
